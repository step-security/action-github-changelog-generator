...
UPSTREAM="janheinrichmerker/action-github-changelog-generator"
...
# Pass the GitHub token via env var (CHANGELOG_GITHUB_TOKEN) instead of --token
# so it doesn't appear in process listings (ps / /proc/<pid>/cmdline).
if [ -n "$INPUT_TOKEN" ]; then export CHANGELOG_GITHUB_TOKEN="$INPUT_TOKEN"; fi
